...
# Navigate to the repo root (assuming the script is in typescript/examples/swapping-agent)
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"

# Create the directory for emberai-mcp if it doesn't exist
mkdir -p "$SCRIPT_DIR/mcp-tools/emberai-mcp"

# Copy emberai-mcp from the source location to swapping-agent directory
