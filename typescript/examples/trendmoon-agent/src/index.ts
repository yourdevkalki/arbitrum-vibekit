...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    endpoints: {
      ...
      '/social-trend/:symbol': 'GET endpoint for social trend data for a specific token',
      '/social-trend': 'POST endpoint to extract token and get social trend data',
...
app.get('/social-trend/:symbol', async (req, res) => {
  console.error(`Received request to /social-trend/${req.params.symbol} endpoint`);
  try {
...
app.post('/social-trend', async (req, res) => {
  ...
  if (!tokenSymbol) {
    return res.status(400).json({ error: 'Could not identify a token symbol in the query' });
  }
  
  try {
    const trendData = await agent.getSocialTrend(tokenSymbol);
    console.error(`Successfully retrieved social trend data for ${tokenSymbol}`);
    res.json({
      symbol: tokenSymbol,
      data: trendData,
    ...
    console.error(`Error handling /social-trend request for ${tokenSymbol}:`, error);
    res.status(500).json({ 
...
app.post('/extract-token', async (req, res) => {
  ...
  try {
    const tokenSymbol = await agent.extractTokenSymbol(query);
    console.error(`Extracted token symbol from query: ${tokenSymbol || 'None found'}`);
  ...
  try {
    ...
    if (tokenData) {
      ...
      res.status(404).json({ 
        ...
        available_tokens: Array.isArray(alerts) ? alerts.map(a => a.symbol) : [] 
      });
    }
  } catch (error) {
    console.error(`Error handling /filter/${req.params.symbol} request:`, error);
    res.status(500).json({ error: (error as Error).message });
  }
});
...
const main = async () => {
  try {
    ...
    app.listen(availablePort, () => {
      console.error(`Trendmoon MCP Agent Server running on port ${availablePort}`);
