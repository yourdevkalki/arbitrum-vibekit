...
import { executeCancelInvestmentOrder } from './src/tools/cancelInvestmentOrder.js';
import { executePortfolioRebalancing } from './src/tools/portfolioRebalancing.js';
...
import { executeTransactionHistory } from './src/tools/transactionHistory.js';
import { executeInvestmentPerformanceTracking } from './src/tools/investmentPerformanceTracking.js';
import { executeAutomatedRebalancingExecution } from './src/tools/automatedRebalancingExecution.js';
import { executeRiskAlertSystem } from './src/tools/riskAlertSystem.js';
import { executeYieldComparisonTool } from './src/tools/yieldComparisonTool.js';

async function testAllCentrifugeTools() {
    console.log('🚀 Comprehensive Centrifuge MCP Tools Test Suite\n');
    console.log('📊 Testing all 13 available tools with real parameters...\n');
    ...
    try {
        ...
        try {
            ...
            console.log('   📊 Result summary:');
            console.log(`      - Content length: ${investmentStatusResult.content.length} characters`);
        } catch (error) {
            console.log(`   ❌ FAILED: ${error.message}`);
        ...
        try {
            ...
            console.log('   📊 Result summary:');
            console.log(`      - Content length: ${poolAnalysisResult.content.length} characters`);
        } catch (error) {
            console.log(`   ❌ FAILED: ${error.message}`);
        ...
        try {
            ...
            console.log('   📊 Result summary:');
            console.log(`      - Content length: ${investmentOrderResult.content.length} characters`);
        } catch (error) {
            console.log(`   ❌ FAILED: ${error.message}`);
