...
describe("MCP Utils", () => {
  describe("createMcpA2AResponse", () => {
    it("should create a valid MCP response for A2A Task object", () => {
      ...
      expect(contentItem.resource).to.have.property("text");
      expect(contentItem.resource).to.have.property("uri");
      expect(contentItem.resource).to.have.property(
        "mimeType",
        "application/json"
      );

      // Verify the JSON content
      const parsedContent = JSON.parse(contentItem.resource.text);
    ...
    it("should create a valid MCP response for A2A Message object", () => {
      ...
      const contentItem = result.content[0];
      expect(contentItem.type).to.equal("resource");
      expect(contentItem.resource.mimeType).to.equal("application/json");
    ...
    it("should generate unique URIs for different calls", () => {
      ...
      const uri1 = result1.content[0].resource.uri;
      const uri2 = result2.content[0].resource.uri;
    ...
    it("should handle agent names with special characters", () => {
      ...
      const uri = result.content[0].resource.uri;
      expect(uri).to.match(
  ...
  describe("createMcpErrorResponse", () => {
    it("should create a valid error response with message only", () => {
      ...
      expect(contentItem.text).to.equal(errorMessage);
    });
    ...
    it("should create a valid error response with error name prefix", () => {
      ...
      const contentItem = result.content[0];
      expect(contentItem.type).to.equal("text");
      expect(contentItem.text).to.equal(
  ...
  describe("createMcpTextResponse", () => {
    it("should create a valid text response", () => {
      ...
      const contentItem = result.content[0];
      expect(contentItem.type).to.equal("text");
      expect(contentItem.text).to.equal(text);
    });
    ...
    it("should handle multiline text", () => {
      ...
      expect(result.content[0].text).to.equal(text);
    });
  });
