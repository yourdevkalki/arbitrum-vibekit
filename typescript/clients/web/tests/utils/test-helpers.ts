...
export const TestDataGenerator = {
  generatePriceData: (numPoints: number, basePrice = 40000, variance = 5000) => {
    const now = Date.now();
    return {
...
export const TestPatterns = {
  ...
  mockCoinGeckoAPI: async (
    ...
    const { shouldFail = false, status = shouldFail ? 500 : 200, delay = 0 } = options;
...
export const PerformanceHelpers = {
  ...
  checkForMemoryLeaks: async (page: Page, iterations = 5) => {
    ...
    const isIncreasing = measurements.length > 2 && 
      measurements.slice(-3).every((val, i, arr) => i === 0 || val > arr[i - 1]);
...
export const VisualHelpers = {
  ...
  compareChartVisuals: async (page: Page, baselineName: string, currentName: string) => {
...
export const ErrorHelpers = {
  expectErrorMessage: async (page: Page, expectedError: string, timeout = 5000) => {
...
export const DataValidationHelpers = {
  validatePriceDataStructure: (data: any) => {
    ...
    return data.prices.every((price: any) => 
      Array.isArray(price) && 
      price.length === 2 && 
  ...
  sanitizePriceData: (data: any) => {
    ...
    const validPrices = data.prices.filter((price: [number, number]) => 
